...
public class ItemSearchService {
	 ...
	 * @return
	 */
	public EasyUIResult search(String keywords, Integer page, Integer rows)
			throws Exception {
		...
		if (isHighlighting) {
			...
			for (Map.Entry<String, Map<String, List<String>>> highlighting : map
				...
				for (Item item : items) {
					...
					item.setTitle(StringUtils.join(
							highlighting.getValue().get("title"), ""));
					break;
				}
			}
		}
		...
		// 设置总记录数
		easyUIResult.setTotal(Integer.parseInt(queryResponse.getResults()
				.getNumFound() + ""));
